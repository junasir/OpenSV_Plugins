...
class ImgHsvIdentifyUi(JuDialog):

    def __init__(self, parent=None, default_parm=None, combox_list=None, log=None, *args, **kwargs):
        super(ImgHsvIdentifyUi, self).__init__(parent, *args, **kwargs)
        self.setWindowTitle("图像HSV颜色操作")
        self.ui_log = log
    ...
    def _init_ui(self):
        ...
        label_h_min = QLabel(self, text="h_min:")
        self.label_h_min = QLineEdit(self)
        label_h_max = QLabel(self, text="h_max:")
        self.label_h_max = QLineEdit(self)
        label_s_min = QLabel(self, text="s_min:")
        self.label_s_min = QLineEdit(self)
        label_s_max = QLabel(self, text="s_max:")
        self.label_s_max = QLineEdit(self)
        label_v_min = QLabel(self, text="v_min:")
        self.label_v_min = QLineEdit(self)
        label_v_max = QLabel(self, text="v_max:")
        self.label_v_max = QLineEdit(self)
        ...
        self._grid.addWidget(label_h_min, 1, 0)
        self._grid.addWidget(self.label_h_min, 1, 1)
        self._grid.addWidget(label_h_max, 2, 0)
        self._grid.addWidget(self.label_h_max, 2, 1)
        self._grid.addWidget(label_s_min, 3, 0)
        self._grid.addWidget(self.label_s_min, 3, 1)
        self._grid.addWidget(label_s_max, 4, 0)
        self._grid.addWidget(self.label_s_max, 4, 1)
        self._grid.addWidget(label_v_min, 5, 0)
        self._grid.addWidget(self.label_v_min, 5, 1)
        self._grid.addWidget(label_v_max, 6, 0)
        self._grid.addWidget(self.label_v_max, 6, 1)

        self._grid.addWidget(label_output_variable, 7, 0)
        self._grid.addWidget(self.label_output_variable, 7, 1)
        self._grid.addWidget(self.update_btn, 8, 0, 1, 2)
    ...
    def generate_parameters(self):
        ...
        if self.label_h_min.text() != "":
            value_list.append(self.label_h_min.text())
        if self.label_h_max.text() != "":
            value_list.append(self.label_h_max.text())
        if self.label_s_min.text() != "":
            value_list.append(self.label_s_min.text())
        if self.label_s_max.text() != "":
            value_list.append(self.label_s_max.text())
        if self.label_v_min.text() != "":
            value_list.append(self.label_v_min.text())
        if self.label_v_max.text() != "":
            value_list.append(self.label_v_max.text())

        if self.label_output_variable.text() != "":
            output_list.append(self.label_output_variable.text())
        if len(value_list) == 7 and len(output_list) == 1:
    ...
    def _parameters_show(self):
        if len(self._default_parm["value"]) == 7:
            self.label_input_variable.setText(self._default_parm["value"][0])
            self.label_h_min.setText(self._default_parm["value"][1])
            self.label_h_max.setText(self._default_parm["value"][2])
            self.label_s_min.setText(self._default_parm["value"][3])
            self.label_s_max.setText(self._default_parm["value"][4])
            self.label_v_min.setText(self._default_parm["value"][5])
            self.label_v_max.setText(self._default_parm["value"][6])
...
class CalcGraphicsNode(QDMGraphicsNode):
    ...
    def __init__(self, node: 'Node'):
        ...
        if self.default_parm is None:
            self.default_parm = {"object": {"type": "OpenCV", "index": 0},
                                 "operation_file": "JuOpencvBlur", "operation_func": "opencv_hsv_func",
    ...
    def double_click_ui_show(self):
        ...
        parameters_win = ImgHsvIdentifyUi(default_parm=self.default_parm, combox_list=self.combox_list, log=self.user_logger)
...
class JuIpImgHsvIdentify(Node):
    icon = ""
    op_code = "CalcNode_ImgHsvIdentify"
    op_title = "图像HSV颜色识别"
    content_label_objname = "calc_node_img_hsv_identify"
    version = "v0.1"
...
if __name__ == "__main__":
    ...
    window = ImgHsvIdentifyUi(default_parm=default_parm, combox_list=["img"])
    window.show()
    exit(app.exec_())
