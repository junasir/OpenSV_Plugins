...
class ImgHsvSplitUi(JuDialog):

    def __init__(self, parent=None, default_parm=None, combox_list=None, log=None, *args, **kwargs):
        super(ImgHsvSplitUi, self).__init__(parent, *args, **kwargs)
        self.setWindowTitle("图像HSV通道分离")
        self.ui_log = log
    ...
    def _init_ui(self):
        ...
        label_output_variable_h = QLabel(self, text="输出H通道图像变量:")
        self.label_output_variable_h = QLineEdit(self)
        label_output_variable_s = QLabel(self, text="输出S通道图像变量:")
        self.label_output_variable_s = QLineEdit(self)
        label_output_variable_v = QLabel(self, text="输出V通道图像变量:")
        self.label_output_variable_v = QLineEdit(self)
        ...
        self._grid.addWidget(label_output_variable_h, 2, 0)
        self._grid.addWidget(self.label_output_variable_h, 2, 1)
        self._grid.addWidget(label_output_variable_s, 3, 0)
        self._grid.addWidget(self.label_output_variable_s, 3, 1)
        self._grid.addWidget(label_output_variable_v, 4, 0)
        self._grid.addWidget(self.label_output_variable_v, 4, 1)
    ...
    def generate_parameters(self):
        if self.label_input_variable.text() != "" and self.label_output_variable_h.text() != ""\
                and self.label_output_variable_s.text() != "" and self.label_output_variable_v.text() != "":
            self._default_parm["variable_input"] = [self.label_input_variable.text()]
            self._default_parm["value"] = [self.label_input_variable.text()]
            self._default_parm["variable_output"] = [self.label_output_variable_h.text(),
                                                     self.label_output_variable_s.text(),
                                                     self.label_output_variable_v.text()]
            self._parameter_change = False
            self.save_ok = True
            self.close()
        else:
    ...
    def _parameters_show(self):
        ...
        if len(self._default_parm["variable_output"]) == 3:
            self.label_output_variable_h.setText(self._default_parm["variable_output"][0])
            self.label_output_variable_s.setText(self._default_parm["variable_output"][1])
            self.label_output_variable_v.setText(self._default_parm["variable_output"][2])
...
class CalcGraphicsNode(QDMGraphicsNode):
    ...
    def __init__(self, node: 'Node'):
        ...
        if self.default_parm is None:
            self.default_parm = {"object": {"type": "OpenCV", "index": 0},
                                 "operation_file": "JuOpencvHsvSplit", "operation_func": "opencv_hsv_split_func",
    ...
    def double_click_ui_show(self):
        ...
        parameters_win = ImgHsvSplitUi(default_parm=self.default_parm, combox_list=combox_list, log=self.user_logger)
...
class JuIpImgHsvSplit(Node):
    icon = "icons/in.png"
    op_code = "CalcNode_ImgHsvSplit"
    op_title = "图像HSV通道分离"
    content_label_objname = "calc_node_img_hsv_split"
    version = "v0.1"

    def __init__(self, scene, inputs=[(1, "input_img")], outputs=[(3, "h_img"), (3, "s_img"),
                                                                  (3, "v_img")]):
...
if __name__ == "__main__":
    ...
    window = ImgHsvSplitUi(default_parm=default_parm, combox_list=["img"])
    window.show()
    exit(app.exec_())
    pass
