...
class ImgGaussianBlurUi(JuDialog):

    def __init__(self, parent=None, default_parm=None, combox_list=None, log=None, *args, **kwargs):
        super(ImgGaussianBlurUi, self).__init__(parent, *args, **kwargs)
        self.setWindowTitle("图像高斯滤波")
        self.ui_log = log
    ...
    def _init_ui(self):
        ...
        label_thresh = QLabel(self, text="kszie:")
        self.label_thresh = QLineEdit(self)
        label_maxval = QLabel(self, text="sigmaX:")
...
class CalcGraphicsNode(QDMGraphicsNode):
    ...
    def __init__(self, node: 'Node'):
        ...
        if self.default_parm is None:
            self.default_parm = {"object": {"type": "OpenCV", "index": 0},
                                 "operation_file": "JuOpencvGaussianBlur", "operation_func": "opencv_gaussian_blur_func",
    ...
    def double_click_ui_show(self):
        ...
        parameters_win = ImgGaussianBlurUi(default_parm=self.default_parm, combox_list=self.combox_list, log=self.user_logger)
...
class JuIpImgGaussianBlur(Node):
    icon = ""
    op_code = "CalcNode_ImgGaussianBlur"
    op_title = "图像高斯模糊"
    content_label_objname = "calc_node_img_gaussian_blur"
    version = "v0.1"
...
if __name__ == "__main__":
    ...
    window = ImgGaussianBlurUi(default_parm=default_parm, combox_list=["img"])
    window.show()
    exit(app.exec_())
