...
class ImgRgbSplitUi(JuDialog):

    def __init__(self, parent=None, default_parm=None, combox_list=None, log=None, *args, **kwargs):
        super(ImgRgbSplitUi, self).__init__(parent, *args, **kwargs)
        self.setWindowTitle("图像BGR通道分离")
        self.ui_log = log
    ...
    def _init_ui(self):
        ...
        label_output_variable_b = QLabel(self, text="输出B通道图像变量:")
        self.label_output_variable_b = QLineEdit(self)
        label_output_variable_g = QLabel(self, text="输出G通道图像变量:")
        self.label_output_variable_g = QLineEdit(self)
        label_output_variable_r = QLabel(self, text="输出R通道图像变量:")
        self.label_output_variable_r = QLineEdit(self)
        ...
        self._grid.addWidget(label_output_variable_b, 2, 0)
        self._grid.addWidget(self.label_output_variable_b, 2, 1)
        self._grid.addWidget(label_output_variable_g, 3, 0)
        self._grid.addWidget(self.label_output_variable_g, 3, 1)
        self._grid.addWidget(label_output_variable_r, 4, 0)
        self._grid.addWidget(self.label_output_variable_r, 4, 1)
        self._grid.addWidget(self.update_btn, 5, 0, 1, 2)
    ...
    def generate_parameters(self):
        if self.label_input_variable.text() != "" and self.label_output_variable_b.text() != ""\
                and self.label_output_variable_g.text() != "" and self.label_output_variable_r.text() != "":
            self._default_parm["variable_input"] = [self.label_input_variable.text()]
            self._default_parm["value"] = [self.label_input_variable.text()]
            self._default_parm["variable_output"] = [self.label_output_variable_b.text(),
                                                     self.label_output_variable_g.text(),
                                                     self.label_output_variable_r.text()]
            self._parameter_change = False
            self.save_ok = True
            self.close()
        else:
    ...
    def _parameters_show(self):
        if len(self._default_parm["variable_input"]) == 1:
            self.label_input_variable.setText(self._default_parm["variable_input"][0])
        if len(self._default_parm["variable_output"]) == 3:
            self.label_output_variable_b.setText(self._default_parm["variable_output"][0])
            self.label_output_variable_g.setText(self._default_parm["variable_output"][1])
            self.label_output_variable_r.setText(self._default_parm["variable_output"][2])
...
class CalcGraphicsNode(QDMGraphicsNode):
    ...
    def __init__(self, node: 'Node'):
        ...
        if self.default_parm is None:
            self.default_parm = {"object": {"type": "OpenCV", "index": 0},
                                 "operation_file": "JuOpencvBgrSplit", "operation_func": "opencv_bgr_split_func",
                                 "node_input_num": 1, "node_output_num": 3,
                                 "value": [],
                                 "result_flag": False,
                                 "result": {},
                                 "variable_input": [],
    ...
    def double_click_ui_show(self):
        ...
        parameters_win = ImgRgbSplitUi(default_parm=self.default_parm, combox_list=combox_list, log=self.user_logger)
...
class JuIpImgBgrSplit(Node):
    icon = "icons/in.png"
    op_code = "CalcNode_ImgBgrSplit"
    op_title = "图像BGR通道分离"
    content_label_objname = "calc_node_img_bgr_split"
    version = "v0.1"

    def __init__(self, scene, inputs=[(1, "input_img")], outputs=[(3, "b_img"), (3, "g_img"),
                                                                  (3, "r_img")]):
...
if __name__ == "__main__":
    ...
    window = ImgRgbSplitUi(default_parm=default_parm, combox_list=["img"])
    window.show()
    exit(app.exec_())
    pass
